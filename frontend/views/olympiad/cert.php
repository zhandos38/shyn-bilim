...
<div id="test" class="pb--60 pt--60">
    <div class="container test-app__container">
        <div class="mt--10">
            <a class="rbt-btn btn-gradient" href="<?= Url::to(['/olympiad/get-cert', 'id' => $testAssignment->id]) ?>" download>
                <?= Yii::t('app', 'Получить сертификат/диплом') ?>
            </a>
        </div>
        <?php if ($testAssignment->olympiad->type === Olympiad::TYPE_STUDENT): ?>
        <div class="mt--10">
            <a class="rbt-btn btn-gradient" href="<?= Url::to(['/olympiad/get-cert-thank-leader', 'id' => $testAssignment->id]) ?>" download>
