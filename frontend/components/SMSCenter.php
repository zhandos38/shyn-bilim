...
// либо массив (<id>, -<код ошибки>) в случае ошибки
    ...
    public function send($phones, $message, $translit = 1, $time = 0, $id = 0, $format = 0, $sender = false, $query = "", $files = array())
    {
        static $formats = array(1 => "flash=1", "push=1", "hlr=1", "bin=1", "bin=2", "ping=1", "mms=1", "mail=1", "call=1", "viber=1", "soc=1");

        $m = $this->smsc_send_cmd("send", "cost=3&phones=".urlencode($phones)."&mes=".urlencode($message)."&rus=5&max_parts=1".
...
// возвращает массив (<стоимость>, <количество sms>) либо массив (0, -<код ошибки>) в случае ошибки
    ...
    public function get_sms_cost($phones, $message, $translit = 0, $format = 0, $sender = false, $query = "")
    {
        ...
        $m = $this->smsc_send_cmd("send", "cost=1&phones=".urlencode($phones)."&mes=".urlencode($message).
            ($sender === false ? "" : "&sender=".urlencode($sender)).
...
// либо массив (0, -<код ошибки>) в случае ошибки
    ...
    {
        $m = $this->smsc_send_cmd("status", "phone=".urlencode($phone)."&id=".urlencode($id)."&all=".(int)$all);
...
// возвращает баланс в виде строки или false в случае ошибки
    ...
    {
        $m = $this->smsc_send_cmd("balance"); // (balance) или (0, -error)
...
// Функция вызова запроса. Формирует URL и делает 5 попыток чтения через разные подключения к сервису
    ...
    private function smsc_send_cmd($cmd, $arg = "", $files = array())
    {
        ...
        do {
            ...
            $ret = $this->smsc_read_url($url, $files, 3 + $i);
        }
        while ($ret == "" && $i < 5);
...
// curl или fsockopen (только http) или включена опция allow_url_fopen для file_get_contents
    ...
    private function smsc_read_url($url, $files, $tm = 5)
    {
        $ret = "";
